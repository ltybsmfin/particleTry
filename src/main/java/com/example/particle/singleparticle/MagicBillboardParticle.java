...
public abstract class MagicBillboardParticle extends Particle {
    public float offsetY;

    protected float scale = 0.1F * (this.random.nextFloat() * 0.5F + 0.5F) * 2.0F;

    protected MagicBillboardParticle(ClientWorld clientWorld, double d, double e, double f) {
        super(clientWorld, d, e, f);
    }

    protected MagicBillboardParticle(ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        IntStream.range(0, 10).forEach(i -> { // 注意移除了 .parallel()
            float baseOffset = i * 1f;
            for (int j = 0; j < 10; j++) {
                float offset = baseOffset + j * 0.1f;
                this.method_60373(vertexConsumer, camera, quaternionf, tickDelta, offset, offsetY);
            }
        });
        //this.method_60373(vertexConsumer, camera, quaternionf, tickDelta);
    }

    protected void method_60373(VertexConsumer vertexConsumer, Camera camera, Quaternionf quaternionf, float f, float OffsetX, float OffsetY) {
        Vec3d vec3d = camera.getPos();
        float g = (float)(MathHelper.lerp((double)f, this.prevPosX, this.x) - vec3d.getX()) + OffsetX;
        float h = (float)(MathHelper.lerp((double)f, this.prevPosY, this.y) - vec3d.getY()) + offsetY;
